package org.gg.library.widgets;
...
public class StateLayout extends FrameLayout {
    ...
    public interface OnEmptyContentRetryListener {
        void onEmptyContentRetry(); // 内容为空时点击重试
    }

    public static final int STATE_LOADING = 8602; // LOADING_STATE

    public static final int STATE_EMPTY_CONTENT = 8603; // EMPTY_CONTENT_STATE

    public static final int STATE_ERROR = 8604; // ERROR_STATE

    public static final int STATE_CONTENT = 8605; // CONTENT_STATE

    protected int mLoadingLayoutId = -1; // 加载布局 id

    protected int mEmptyContentLayoutId = -1; // 空布局 id

    protected int mErrorLayoutId = -1; // 错误布局 id

    private SparseArray<ViewStub> mStateLayoutStubItems = new SparseArray<>(); // 懒加载子布局 需要时再进行填充

    private SparseArray<View> mCurrentViewItems = new SparseArray<>(); // 记录已经加载过的子布局

    private ArrayList<ItemView> mSavedItemViews = new ArrayList<>(); // 用于恢复布局内容时使用

    private int mCurState; // 当前状态

    private boolean mEnableContentAnim = true; // 状态切换时视图渐显动画

    protected boolean mHasInit = false; // 是否已经加载过

    protected OnEmptyContentRetryListener mEmptyContentRetryListener; // 空视图重试按钮监听
    protected OnErrorRetryListener mErrorRetryListener; // 错误视图重试按钮监听

    protected View mContentView; // 内容视图（置于该自定义布局标签内的子视图）
    ...
    public Button getEmptyRetryButton() {
        return (Button) mEmptyRetryButton;
    }
    ...
    private void initEmptyContentListener(View itemView) {
        ...
        mEmptyRetryButton = itemView.findViewById(R.id.sl_empty_content_retry);
        if (mEmptyRetryButton != null) {
            mEmptyRetryButton.setOnClickListener(view -> {
    ...
    static class ItemView implements Parcelable {
        ...
        static final Creator<ItemView> CREATOR = new Creator<ItemView>() {
            @Override
