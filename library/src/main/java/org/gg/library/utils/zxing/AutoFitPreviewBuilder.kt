...
class AutoFitPreviewBuilder private constructor(config: PreviewConfig,
    ...
    private val displayListener = object : DisplayManager.DisplayListener {
        ...
        override fun onDisplayChanged(displayId: Int) {
            val viewFinder1 = viewFinderRef.get() ?: return
            if (displayId == viewFinderDisplay) {
                val display = displayManager.getDisplay(displayId)
                val rotation = getDisplaySurfaceRotation(display)
                updateTransform(viewFinder1, rotation, bufferDimens, viewFinderDimens)
            }
        }
    }
    ...
    init {
        ...
        useCase.onPreviewOutputUpdateListener = Preview.OnPreviewOutputUpdateListener {
            val viewFinder2 = viewFinderRef.get() ?: return@OnPreviewOutputUpdateListener

            // To update the SurfaceTexture, we have to remove it and re-add it
            val parent = viewFinder2.parent as ViewGroup
            parent.removeView(viewFinder2)
            parent.addView(viewFinder2, 0)

            viewFinder2.surfaceTexture = it.surfaceTexture
            bufferRotation = it.rotationDegrees
            val rotation = getDisplaySurfaceRotation(viewFinder2.display)
            updateTransform(viewFinder2, rotation, it.textureSize, viewFinderDimens)
        }

        // Every time the provided texture view changes, recompute layout
        viewFinder.addOnLayoutChangeListener { view, left, top, right, bottom, _, _, _, _ ->
            val viewFinder3 = view as TextureView
            val newViewFinderDimens = Size(right - left, bottom - top)
            val rotation = getDisplaySurfaceRotation(viewFinder3.display)
            updateTransform(viewFinder3, rotation, bufferDimens, newViewFinderDimens)
        }
